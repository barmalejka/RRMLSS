...
images = [[],[]]
for f in glob.glob("".join([path,"/*"])): #"dev_dataset/*"
    images[0].append(Image.open(f).resize((block_size, block_size), Image.BILINEAR))
    images[1].append(f.replace("dev_dataset\\", ""))
...
result_fin = np.array(list(filter(lambda x: (x[0] >= 0 and x[0] <= threshold ), result)))[:, 1:]
result_fin.sort()
